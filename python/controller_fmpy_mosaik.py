...
class tap_controller(mosaik_api.Simulator):
    ...
    def create(self, num, model, v_low = 0.98, v_up = 1.01):
        ...
        for i in range(num):
            ...
            self.data[eid] = {'tap': self._entities[eid].getInteger([self.vr_outputs[0]])}
    ...
    def step(self, time, inputs, max_advance):
        ...
        for eid, input_data in inputs.items():
            ...
            if True is self.is_responsive[eid]:
                if u3 is not None: 
                # or v4 is not None:
                    new_tap = self.tap_change(eid, u3, communication_step_size )
                    print ("new tap from func", new_tap)
                    self.tap_pos=new_tap
    ...
    def tap_change( self, eid, u3, step_size ):
        ...
        # assert status is not None
        tap_new = self._entities[eid].getInteger(self.vr_outputs)
        print("tap_new="+str(tap_new))
