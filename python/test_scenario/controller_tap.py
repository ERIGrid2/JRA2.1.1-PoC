...
class controller_tap(Fmi2Slave):
    """ 
        Class to model a basic voltage controller to alter 
        transfomer tap position.
    ...
    def __init__(self, **kwargs):
        ...
        self.register_variable(Integer("tap", causality=Fmi2Causality.output))
        print("Controller co-simulation: INIT DONE")
    
    def do_step(self, current_time, step_size):
        """ Function to update state of controller and change tap position if required.
        Parameters
        ----------
        comm_point: float
